# Starks Sprutmåleri AB: a lead generation WordPress site built around CRO

A lead generation website for **Starks Sprutmåleri AB Roofing**, a made-up roofing company. It was built as a portfolio piece to show how I approach a lead-gen redesign in WordPress: conversion rate optimisation (CRO) basics first, a quote form that captures and tracks every lead, and old content moved over cleanly.

> Starks Sprutmåleri AB Roofing is a fictional company. The copy, prices, reviews and figures are invented and labelled as samples on the site.

**Try it in your browser (no install):** [Open in WordPress Playground](https://playground.wordpress.net/?blueprint-url=https://raw.githubusercontent.com/hugooblak/starkssprutmaleri-demo/main/blueprint.json)
...
## Lead capture: the Starks Sprutmåleri AB Leads plugin
...
| Part | What it is |
|---|---|
| `theme/starkssprutmaleri` | Block theme: `theme.json`, HTML templates, PHP patterns. Two page templates: *Landing* (full-width sections) and *Focus* (no menu, for the funnel) |
| `plugins/starkssprutmaleri-leads` | Quote form block, leads inbox, export, tracking. A plugin, so leads survive a redesign |
| `plugins/starkssprutmaleri-demo-notes` | Demo bar and CRO notes only. **Delete on a real site.** Notes aren't stored in page content |
...
| Add a section | "+" → Patterns → **Starks Sprutmåleri AB sections** (hero with form, trust strip, steps, services, price guide, reviews, comparison, promise, FAQ, final call to action) |
| Start a new landing page | Pages → Add new → pick a **Starks Sprutmåleri AB page layout** → set template to *Landing page* |
...
| Change the answer choices | `plugins/starkssprutmaleri-leads/includes/fields.php` (one list feeds the form, inbox and export) |
...
1. Copy `theme/starkssprutmaleri` into `wp-content/themes/` and activate it.
...
plugins/starkssprutmaleri-leads/
...
plugins/starkssprutmaleri-demo-notes/ Demo bar and CRO notes (remove on a real site)
proof/                        Lighthouse reports, axe results, screenshots
```

## Licence

GPL-2.0-or-later, like WordPress. Font: SIL Open Font License (see `theme/starkssprutmaleri/assets/fonts/OFL.txt`).
