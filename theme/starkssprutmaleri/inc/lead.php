 ...
 * or photo. They call st_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Starks Sprutmåleri AB.
 *
 ...
 * @return string
 */
function st_sample_note() {
	return st_lead( 'ar_riktig' ) ? '' : ' <span class="st-sample">(platshållare)</span>';
}
 ...
 * @return string File name for st_lead_img().
 */
function st_lead_bild( $i ) {
	$bilder = (array) st_lead( 'bilder', array() );
 ...
 * @return string
 */
function st_ring_label() {
	$namn = st_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, st_lead( 'telefon' ) ) : 'Ring ' . st_lead( 'telefon' );
}
