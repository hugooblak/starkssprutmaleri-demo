/* Starks Sprutmåleri AB Leads, on every page (about 1 KB):
...
(function () {
	document.addEventListener('click', function (e) {
		...
		if (!a) return;
		var where = a.closest('.st-mobile-cta') ? 'mobile_bar' : a.closest('header') ? 'header' : a.closest('footer') ? 'footer' : 'content';
		var push = function (o) { (window.dataLayer = window.dataLayer || []).push(o); };
		if (a.protocol === 'tel:') push({ event: 'npl_click_call', location: where });
		else if (a.closest('.st-track-cta')) push({ event: 'npl_click_cta', location: where, text: a.textContent.trim() });
	});
})();
