 ...
 * @package Starks Sprutmåleri ABLeads
 */
...
if ( 'start' === $npl_variant ) :
	...
	$npl_button     = ! empty( $attributes['buttonText'] ) ? $attributes['buttonText'] : __( 'Räkna på mitt tak', 'starkssprutmaleri-leads' );
	?>
	<div <?php echo $npl_wrapper; // phpcs:ignore WordPress.Security.EscapeOutput ?>>
		<form class="npl-start" method="get" action="<?php echo esc_url( $npl_action ); ?>" novalidate data-npl-start>
			<label class="npl-label" for="<?php echo esc_attr( $npl_uid ); ?>-zip"><?php esc_html_e( 'Ditt postnummer', 'starkssprutmaleri-leads' ); ?></label>
			...
			<p class="npl-fineprint"><?php esc_html_e( 'Steg 1 av 4 · dina uppgifter lämnas aldrig vidare', 'starkssprutmaleri-leads' ); ?></p>
		</form>
	</div>
	<?php
	return;
...
$npl_steps = array(
	1 => __( 'Var ligger huset?', 'starkssprutmaleri-leads' ),
	2 => __( 'Vad behöver du hjälp med?', 'starkssprutmaleri-leads' ),
	3 => __( 'När vill du ha det gjort?', 'starkssprutmaleri-leads' ),
	4 => __( 'Vart skickar vi offerten?', 'starkssprutmaleri-leads' ),
);
?>
	...
	<form class="npl-steps" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" novalidate data-npl-steps data-total="4" data-start="<?php echo (int) $npl_start; ?>">
		...
		<?php if ( $npl_errors ) : ?>
			<div class="npl-summary" role="alert" tabindex="-1" data-npl-summary>
				<p><strong><?php esc_html_e( 'Kolla de här svaren:', 'starkssprutmaleri-leads' ); ?></strong></p>
				<ul>
		...
		<div class="npl-progress" data-npl-progress hidden>
			<p class="npl-progress-text" aria-live="polite" data-npl-progress-text><?php echo esc_html( sprintf( /* translators: %d: step number */ __( 'Steg %d av 4', 'starkssprutmaleri-leads' ), $npl_start ) ); ?></p>
		...
		<fieldset class="npl-step" data-step="1" data-name="zip" tabindex="-1">
			...
			<label class="npl-label" for="<?php echo esc_attr( $npl_uid ); ?>-zip"><?php esc_html_e( 'Postnummer', 'starkssprutmaleri-leads' ); ?></label>
		...
		foreach ( array( 2 => 'service', 3 => 'timeline' ) as $npl_n => $npl_field ) :
			...
			<fieldset class="npl-step" data-step="<?php echo (int) $npl_n; ?>" data-name="<?php echo esc_attr( $npl_field ); ?>" tabindex="-1" id="<?php echo esc_attr( $npl_uid . '-' . $npl_field ); ?>" aria-describedby="<?php echo esc_attr( $npl_uid . '-' . $npl_field ); ?>-err"<?php echo isset( $npl_errors[ $npl_field ] ) ? ' aria-invalid="true"' : ''; ?>>
				...
				<p class="npl-hint npl-auto-hint"><?php esc_html_e( 'När du väljer går du vidare till nästa fråga.', 'starkssprutmaleri-leads' ); ?></p>
		...
		<fieldset class="npl-step" data-step="4" data-name="contact" tabindex="-1">
			...
			<?php
			$npl_contact = array(
				'name'  => array( __( 'Förnamn', 'starkssprutmaleri-leads' ), 'text', 'given-name', '' ),
				'phone' => array( __( 'Mobilnummer', 'starkssprutmaleri-leads' ), 'tel', 'tel-national', __( 'För att boka tid för besiktningen.', 'starkssprutmaleri-leads' ) ),
				'email' => array( __( 'E-post', 'starkssprutmaleri-leads' ), 'email', 'email', __( 'Hit skickar vi den skriftliga offerten.', 'starkssprutmaleri-leads' ) ),
			);
		...
		<div class="npl-nav">
			<button type="button" class="npl-back" data-npl-back hidden><span aria-hidden="true">←</span> <?php esc_html_e( 'Back', 'starkssprutmaleri-leads' ); ?></button>
			<button type="button" class="npl-btn wp-element-button" data-npl-next hidden><?php esc_html_e( 'Next', 'starkssprutmaleri-leads' ); ?> <span aria-hidden="true">→</span></button>
			<button type="submit" class="npl-btn wp-element-button" data-npl-submit><?php esc_html_e( 'Skicka min förfrågan', 'starkssprutmaleri-leads' ); ?></button>
		</div>
