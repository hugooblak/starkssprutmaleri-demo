...
add_filter(
	...
	function ( $views ) {
		...
		$views['npl_export'] = '<a href="' . esc_url( $url ) . '" class="button" style="margin-left:6px">' . esc_html__( 'Export CSV', 'starkssprutmaleri-leads' ) . '</a>';
		return $views;
	}
);
...
add_action(
	...
	function () {
		if ( ! current_user_can( 'export_npl_leads' ) ) {
			wp_die( esc_html__( 'Du har inte behörighet att exportera förfrågningar.', 'starkssprutmaleri-leads' ), 403 );
		}
