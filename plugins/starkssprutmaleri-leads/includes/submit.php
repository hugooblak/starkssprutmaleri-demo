...
function npl_validate( $in ) {
	...
	if ( 5 !== strlen( $v['zip'] ) ) {
		$errors['zip'] = __( 'Skriv ditt postnummer, till exempel 123 45.', 'starkssprutmaleri-leads' );
	}
	if ( ! isset( $choices['service'][ $v['service'] ] ) ) {
		$errors['service'] = __( 'Välj vad du behöver hjälp med.', 'starkssprutmaleri-leads' );
	}
	if ( ! isset( $choices['timeline'][ $v['timeline'] ] ) ) {
		$errors['timeline'] = __( 'Välj när du vill ha jobbet gjort.', 'starkssprutmaleri-leads' );
	}
	if ( strlen( $v['name'] ) < 2 ) {
		$errors['name'] = __( 'Skriv ditt förnamn.', 'starkssprutmaleri-leads' );
	}
	...
	if ( 10 !== strlen( $digits ) ) {
		$errors['phone'] = __( 'Skriv ditt mobilnummer, till exempel 070-123 45 67.', 'starkssprutmaleri-leads' );
	} else {
		$v['phone'] = sprintf( '(%s) %s-%s', substr( $digits, 0, 3 ), substr( $digits, 3, 3 ), substr( $digits, 6 ) );
	}
	if ( ! is_email( $v['email'] ) ) {
		$errors['email'] = __( 'Skriv en e-postadress, till exempel namn@exempel.se.', 'starkssprutmaleri-leads' );
	}
...
function npl_handle_submit() {
	...
	if ( is_wp_error( $lead_id ) ) {
		wp_die( esc_html__( 'Något gick fel. Ring oss på 070-123 45 67 så tar vi det direkt.', 'starkssprutmaleri-leads' ), 500 );
	}
