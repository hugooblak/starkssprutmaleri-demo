...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'starkssprutmaleri-leads' ),
					'singular_name' => __( 'Lead', 'starkssprutmaleri-leads' ),
					'edit_item'     => __( 'Förfrågan', 'starkssprutmaleri-leads' ),
					'search_items'  => __( 'Search leads', 'starkssprutmaleri-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'starkssprutmaleri-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'starkssprutmaleri-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'starkssprutmaleri-leads' ),
			'npl_phone'    => __( 'Phone', 'starkssprutmaleri-leads' ),
			'npl_service'  => __( 'Needs', 'starkssprutmaleri-leads' ),
			'npl_timeline' => __( 'When', 'starkssprutmaleri-leads' ),
			'npl_zip'      => __( 'ZIP', 'starkssprutmaleri-leads' ),
			'npl_source'   => __( 'Source', 'starkssprutmaleri-leads' ),
			'npl_status'   => __( 'Status', 'starkssprutmaleri-leads' ),
			'date'         => __( 'Received', 'starkssprutmaleri-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'starkssprutmaleri-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'starkssprutmaleri-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'starkssprutmaleri-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'starkssprutmaleri-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'starkssprutmaleri-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'starkssprutmaleri-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'starkssprutmaleri-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'starkssprutmaleri-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'starkssprutmaleri-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'starkssprutmaleri-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'starkssprutmaleri-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'starkssprutmaleri-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
